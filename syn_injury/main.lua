		...
		if DoesEntityExist(ped) --[[ and GetEntityHealth(ped) >= 175  ]]then
			...
			if Config.resetwalk then
				...
				if currentHealth ~= oldBodyHealth then
					if not nothurt or veryhurt and GetEntityHealth(ped) >= 175 --[[ and not hurt and ]] and (currentHealth > oldBodyHealth) and ((currentHealth - oldBodyHealth) >= Config.WalkResetHealingRequired) then
						setNotHurt()
